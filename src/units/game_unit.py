class GameUnit:
    ...
    def deplacement():
        if self.y > 0:
            bestID = redId[0]
            for i in range(3):
                if sqrt((self.x - redCoordinates[i][0]) ** 2 + (self.y - redCoordinates[i][1]) ** 2) < sqrt(
                        (self.x - redCoordinates[id.index[bestID]][0]) ** 2 + (
                                self.y - redCoordinates[id.index[bestID]][1]) ** 2):
                    if redType[id.index[
                        bestID]] == "tower":  # Ce i in range sert à identifier l'ID de la tour courronnée la plus proche
                        bestID = redId[i]
        ...
        for i in range(len(redId)):
            if sqrt((self.x - redCoordinates[i][0]) ** 2 + (self.y - redCoordinates[i][1]) ** 2) < sqrt(
                    (self.x - redCoordinates[id.index[bestID]][0]) ** 2 + (
                            self.y - redCoordinates[id.index[bestID]][1]) ** 2):
                if redType[id.index[
                    bestID]] != "tower":  # Ce i in range sert à identifier l'ID de l'unité ennemie la plus proche
                    if sqrt((self.x - redCoordinates[id.index[bestID]][0]) ** 2 + (
                            self.y - redCoordinates[id.index[bestID]][
                        1]) ** 2) < 200:  # portée de reconnaissance , au dela de 200 de distance l'unité est ignorée
